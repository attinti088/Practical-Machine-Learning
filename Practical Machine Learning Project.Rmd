...
## Reproducibility  
Below are packages used and the seed value, you need to use the same version of package to reproduce the results

`: To install, for instance, the `rattle` package in R, run this command: `install.packages("rattle")`.  
...
```{r warning=FALSE, error=FALSE}
setwd("H:/RProjects/Practical Machine Learning")
```
...
1. Removing the <b>Near Zero Variance</b> Variables.  
```{r warning=FALSE, error=FALSE}
...
Now, we apply the <b>Random Forest</b> model to the original testing data set downloaded from the data source.  
```{r warning=FALSE, error=FALSE}
rm(accuracy)
rm(ose)
